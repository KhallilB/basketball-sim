...
export class PositionalPossessionEngine {
  ...
  private calculateEPV(ctx: PositionalCtx, situation: GameSituation): Record<Action, number> {
    ...
    if (!playerPos) {
      ...
      return {
        drive: 0.5, pullup: 0.2, catchShoot: 0.3,
        pnrAttack: 0.35, pnrPass: 0.15, post: 0.1, reset: 0.05
      };
    }
  ...
  private resolve(action: Action, ctx: PositionalCtx): { kind: string; [key: string]: any } {
    ...
    const defenderId = ctx.coordinator.getMatchup(ctx.with.id, ctx.state.defensiveAssignments);
    const rDef = defenderId ? 
      ctx.def.players.find(p => p.id === defenderId)?.ratings :
      ctx.def.players[0].ratings; // Fallback to first defender
    ...
    if (action === 'drive') {
      const openLanes = ctx.state.spacing.openLanes;
      const driveAngle = defenderPos ? 
        Math.atan2(defenderPos.y - playerPos!.y, defenderPos.x - playerPos!.x) : 0;
    ...
    if (action === 'pullup' || action === 'catchShoot') {
      ...
      const contest = defenderPos && playerPos ? 
        Math.max(0, 1 - distance(playerPos, defenderPos) / 6) : 0.2; // Lower default contest
      ...
      console.log(`Shot: ${zone}, Q=${Q.toFixed(2)}, contest=${contest.toFixed(2)}, P=${prediction.p.toFixed(3)}, made=${make}`);
    ...
    if (action === 'pnrPass') {
      ...
      const pressure = defenderPos && playerPos ? 
        Math.max(0, 1 - distance(playerPos, defenderPos) / 8) : 0.5;
  ...
  private calculateSpacing(offFormation: any, defFormation: any) {
    ...
    return {
      openLanes: calculateOpenLanes(
        offFormation.ballPosition,
        offPositions,
        defPositions,
        true
      ),
      ballMovement: 0, // Starts at 0, increases with ball movement
      shotQuality: calculateShotQuality(
        offFormation.ballPosition,
        defPositions[0] || null,
        true
      )
  ...
  private advanceFatigue(ctx: PositionalCtx, action: Action) {
    const per = PARAMS.fatigue.perAction + (action === 'pullup' || action === 'catchShoot' ? PARAMS.fatigue.perShot : 0);
  ...
  private resolveRebound(ctx: PositionalCtx): Id {
    // Enhanced rebound resolution with positioning
    const o = ctx.off.players[0];
    const d = ctx.def.players[0];
    
    // TODO: Use actual positioning for rebound calculation
