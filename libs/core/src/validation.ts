...
export class RatingError extends ValidationError {
  constructor(rating: number, field: string) {
    super(`Invalid rating ${rating} for ${field}. Must be between ${CONFIG.VALIDATION.RATING_RANGE[0]} and ${CONFIG.VALIDATION.RATING_RANGE[1]}`, field, rating);
...
export function validateRatings(ratings: Ratings): Ratings {
  ...
  const requiredFields: (keyof Ratings)[] = [
    'three', 'mid', 'finishing', 'ft', 'pass', 'handle', 'post', 'roll', 'screen',
    'onBallDef', 'lateral', 'rimProt', 'steal', 'speed', 'accel', 'strength', 
    'vertical', 'rebound', 'height', 'wingspan', 'iq', 'discipline', 
    'consistency', 'clutch', 'stamina', 'durability'
  ];
  ...
  for (const field of requiredFields) {
    ...
    if (field === 'height') {
      if (ratings.height < 152 || ratings.height > 229) { // 5'0" to 7'6" in cm
        throw new ValidationError(`Invalid height ${ratings.height} cm. Must be between 152-229 cm`, 'height', ratings.height);
      }
      validatedRatings.height = ratings.height;
    } else if (field === 'wingspan') {
      if (ratings.wingspan < 152 || ratings.wingspan > 254) { // 5'0" to 8'4" in cm
        throw new ValidationError(`Invalid wingspan ${ratings.wingspan} cm. Must be between 152-254 cm`, 'wingspan', ratings.wingspan);
      }
...
export function validateTendency(tendency: number, fieldName: string): number {
  ...
  if (tendency < min || tendency > max) {
    throw new ValidationError(`Invalid tendency ${tendency} for ${fieldName}. Must be between ${min} and ${max}`, fieldName, tendency);
  }
...
export function validateTendencies(tendencies: Tendencies): Tendencies {
  ...
  if (!Array.isArray(validated.threeStyle) || validated.threeStyle.length !== 2) {
    throw new ValidationError('threeStyle tendencies must be an array of 2 numbers', 'threeStyle', validated.threeStyle);
  }
...
export function validateProbability(probability: number, fieldName: string): number {
  if (typeof probability !== 'number' || isNaN(probability)) {
    throw new ValidationError(`Invalid probability ${probability} for ${fieldName}. Must be a number`, fieldName, probability);
  }
  
  const [min, max] = CONFIG.VALIDATION.PROBABILITY_RANGE;
  if (probability < min || probability > max) {
    throw new ValidationError(`Invalid probability ${probability} for ${fieldName}. Must be between ${min} and ${max}`, fieldName, probability);
  }
...
export function validateFatigue(fatigue: number, playerId: string): number {
  if (typeof fatigue !== 'number' || isNaN(fatigue)) {
    throw new ValidationError(`Invalid fatigue ${fatigue} for player ${playerId}. Must be a number`, 'fatigue', fatigue);
  }
  
  const [min, max] = CONFIG.VALIDATION.FATIGUE_RANGE;
  if (fatigue < min || fatigue > max) {
    throw new ValidationError(`Invalid fatigue ${fatigue} for player ${playerId}. Must be between ${min} and ${max}`, 'fatigue', fatigue);
  }
