...
import { OffensiveSystemManager, OffensiveContext } from './offensive-system.js';

/**
 * Manages player formations and positioning using advanced offensive systems
 */
export class FormationManager {
  private offensiveSystem = new OffensiveSystemManager();
  /**
   * Initialize advanced offensive formation using basketball principles
   */
  createOffensiveFormation(
    ...
    // Select appropriate offensive set based on team and game state
    const offensiveSet = this.offensiveSystem.selectOffensiveSet(team, gameTimeElapsed, scoreDiff, shotClock);

    // Create offensive context
    const context: OffensiveContext = {
      set: offensiveSet,
      ballHandler: team.players[0].id, // Start with first player
      gameTimeElapsed,
      scoreDiff,
      shotClock
    };

    // Generate formation using advanced offensive system
    return this.offensiveSystem.createOffensiveFormation(team, isHomeTeam, context);
  }
   ...
   */
  updateFormationAfterBallMovement(formation: Formation, newBallPosition: Position): Formation {
   ...
   */
  optimizeOffensiveSpacing(formation: Formation): Formation {
  ...
  private getFullCourtPressPositions(offensiveFormation: Formation, isHomeTeam: boolean): Position[] {
    ...
    // Deny passing lanes
    Object.values(offensiveFormation.players)
      .slice(2)
      .forEach((offPos, index) => {
        positions.push({
