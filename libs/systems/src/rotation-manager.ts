...
export class RotationManager {
  ...
  shouldSubstitute(
    ...
    if (minutesPlayed >= targetMinutes * 0.6) { // After 60% of target minutes
  ...
  private calculatePlayerOverall(player: Player): number {
    const weights = {
      three: 0.08, mid: 0.08, finishing: 0.10, ft: 0.03, pass: 0.06,
      handle: 0.08, post: 0.05, roll: 0.04, screen: 0.03, onBallDef: 0.08,
      lateral: 0.06, rimProt: 0.06, steal: 0.04, speed: 0.06,
      strength: 0.04, vertical: 0.04, rebound: 0.07, iq: 0.08,
      discipline: 0.03, consistency: 0.05, clutch: 0.03, stamina: 0.03,
      heightIn: 0.01, wingspanIn: 0.01
    };
    
    let overall = 0;
    for (const [key, weight] of Object.entries(weights)) {
      overall += player.ratings[key as keyof typeof player.ratings] * weight;
    }
